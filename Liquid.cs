    ...
    public class Liquid
    {
        ...
        public static double QuickWater(int verbose = 0, int minY = -1, int maxY = -1)
        {
            ...
            for (int i = maxY; i >= minY; i--)
            {
                ...
                if (verbose > 0)
                {
                    num3 = ((float) (maxY - i)) / ((float) ((maxY - minY) + 1));
                    num3 /= (float) verbose;
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    for (int k = num5; k != num6; k += num7)
                    {
                        ...
                        while (((flag3 && (x > 3)) && (x < (Main.maxTilesX - 3))) && (y < (Main.maxTilesY - 3)))
                        {
                            ...
                            {
                                int num14 = 0xff - Main.tile[x, y + 1].liquid;
                                if (num14 > liquid)
                                {
                                    num14 = liquid;
                                }
                                Tile tile1 = Main.tile[x, y + 1];
                                tile1.liquid = (byte) (tile1.liquid + ((byte) num14));
                                liquid = (byte) (liquid - ((byte) num14));
                                if (liquid <= 0)
                                {
                                    num++;
                                    break;
                            ...
                            if (num13 == 0)
                            {
                                if ((Main.tile[x + num9, y].liquid == 0) && ((!Main.tile[x + num9, y].active || !Main.tileSolid[Main.tile[x + num9, y].type]) || Main.tileSolidTop[Main.tile[x + num9, y].type]))
                                {
                                    num13 = num9;
                                }
                                else if ((Main.tile[x - num9, y].liquid == 0) && ((!Main.tile[x - num9, y].active || !Main.tileSolid[Main.tile[x - num9, y].type]) || Main.tileSolidTop[Main.tile[x - num9, y].type]))
                                {
                                    num13 = -num9;
                                }
                            }
                            if ((num13 != 0) && ((Main.tile[x + num13, y].liquid == 0) && ((!Main.tile[x + num13, y].active || !Main.tileSolid[Main.tile[x + num13, y].type]) || Main.tileSolidTop[Main.tile[x + num13, y].type])))
                            {
                                flag3 = true;
                                x += num13;
                            }
        ...
        public static void UpdateLiquid()
        {
                    ...
                    while ((panicY >= 3) && (num < 5))
                    {
                        ...
                        if (panicY < 3)
                        {
                            ...
                            if (Main.netMode == 2)
                            {
                                ...
                                {
                                    num3 = 0;
                                    while (num3 < Main.maxSectionsX)
                                    {
                                        for (int j = 0; j < Main.maxSectionsY; j++)
                                        {
                                            Netplay.serverSock[i].tileSection[num3, j] = false;
                                        }
                                        num3++;
                                    }
                                }
            ...
            if (quickFall)
            {
                for (num3 = num6; num3 < numLiquid; num3++)
                {
                    Main.liquid[num3].delay = 10;
                    Main.liquid[num3].Update();
                    Main.tile[Main.liquid[num3].x, Main.liquid[num3].y].skipLiquid = false;
                }
            }
            else
            {
                num3 = num6;
                while (num3 < numLiquid)
                {
                    if (!Main.tile[Main.liquid[num3].x, Main.liquid[num3].y].skipLiquid)
                    {
                        Main.liquid[num3].Update();
                    }
                    else
            ...
            {
                wetCounter = 0;
                for (num3 = Liquid.numLiquid - 1; num3 >= 0; num3--)
                {
                    if (Main.liquid[num3].kill > 3)
                    {
                        DelWater(num3);
                    }
                }
                ...
                for (num3 = 0; num3 < numLiquidBuffer; num3++)
                {
