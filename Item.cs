...
namespace Terraria
{
  public class Item
  {
    ...
    public bool checkMat()
    {
      ...
      else
      {
        for (int index1 = 0; index1 < Recipe.numRecipes; ++index1)
        {
          ...
          {
            if (this.name == Main.recipe[index1].requiredItem[index2].name)
            {
              this.material = true;
              return true;
            }
    ...
    {
      this.owner = Main.netMode != 1 && Main.netMode != 2 ? Main.myPlayer : (int) byte.MaxValue;
      this.material = false;
      this.vanity = false;
      this.mana = 0;
      this.wet = false;
      ...
      else if (this.type == 261)
      {
        ...
        this.consumable = true;
        this.value = 1000;
        this.toolTip = "Its smiling, might be a good snack";
      }
      ...
      if (this.active)
      {
        ...
        if (!this.beingGrabbed)
        {
          ...
          if (this.owner == Main.myPlayer && this.lavaWet)
          {
            ...
            this.stack = 0;
            if (Main.netMode != 0)
              NetMessage.SendData(21, -1, -1, "", i, 0.0f, 0.0f, 0.0f);
          }
        ...
        if (Main.netMode == 2 && this.owner != Main.myPlayer)
        {
          ...
          {
            this.release = 0;
            NetMessage.SendData(39, this.owner, -1, "", i, 0.0f, 0.0f, 0.0f);
          }
        }
        if (this.wet)
    ...
    public static int NewItem(int X, int Y, int Width, int Height, int Type, int Stack = 1, bool noBroadcast = false)
    {
      ...
      else
      {
        ...
        {
          NetMessage.SendData(21, -1, -1, "", index1, 0.0f, 0.0f, 0.0f);
      ...
      if (this.keepTime <= 0)
      {
        ...
        for (int index2 = 0; index2 < (int) byte.MaxValue; ++index2)
        {
          if (this.ownIgnore != index2 && (Main.player[index2].active && Main.player[index2].ItemSpace(Main.item[whoAmI])))
          {
            ...
            if ((double) num2 < (double) (Main.screenWidth / 2 + Main.screenHeight / 2) && ((double) num1 == -1.0 || (double) num2 < (double) num1))
            {
              num1 = num2;
              this.owner = index2;
            }
        ...
        {
          NetMessage.SendData(21, -1, -1, "", whoAmI, 0.0f, 0.0f, 0.0f);
          if (this.active)
            NetMessage.SendData(22, -1, -1, "", whoAmI, 0.0f, 0.0f, 0.0f);
        }
      }
    }
